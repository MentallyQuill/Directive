...
assert.match(liveSmokeSource, /runtimeLedgerProofAvailable\s*=\s*runtimeLedgerView\?\.authoritative === true[\s\S]{0,120}runtimeLedgerView\?\.coreProjectionAvailable === true/);
assert.match(liveSmokeSource, /progressSource:[\s\S]{0,220}'core-runtime-ledger-count'[\s\S]{0,120}'core-runtime-ledger'/);
assert.doesNotMatch(liveSmokeSource, /progressSource:[\s\S]{0,220}'runtime-tracking-count'[\s\S]{0,120}'runtime-tracking'/);
assert.match(liveSmokeSource, /sentRoundCount === 0 \|\| finalCoreIngressOk/);
...
assert.match(liveSmokeSource, /const\s+sidecars\s*=\s*coreSidecarRows\.map\(\(entry\)\s*=>/);
assert.match(liveSmokeSource, /legacySidecarCount:\s*legacySidecarRows\.length/);
assert.doesNotMatch(liveSmokeSource, /coreSidecarRows\.length[\s\S]{0,160}runtimeTracking\?\.sidecarJournal/);
assert.doesNotMatch(liveSmokeSource, /sidecarCount:\s*\(view\?\.campaignState\?\.runtimeTracking\?\.sidecarJournal/);
...
assert.match(liveHarnessSource, /const\s+recoveryJournal\s*=\s*runtimeLedgerProofAvailable[\s\S]{0,160}coreRecoveryJournal/);
assert.doesNotMatch(liveHarnessSource, /const\s+recoveryJournal\s*=[\s\S]{0,180}runtimeTracking\.recoveryJournal/);
assert.match(liveHarnessSource, /sidecarCount:\s*coreSidecars\.length/);
assert.doesNotMatch(liveHarnessSource, /coreSidecars\.length \|\| legacySidecarCount/);
assert.match(liveHarnessSource, /const\s+primaryModelCalls\s*=\s*Array\.isArray\(view\?\.chatNative\?\.modelCalls\)\s*\?\s*view\.chatNative\.modelCalls\s*:\s*\[\]/);
assert.match(liveHarnessSource, /legacyModelCallCount:\s*legacyModelCallTelemetry\.length/);
assert.doesNotMatch(liveHarnessSource, /modelCallCount:\s*view\?\.chatNative\?\.modelCalls\?\.length\s*\|\|\s*view\?\.campaignState\?\.runtimeTracking\?\.modelCallJournal/);
assert.match(liveHarnessSource, /sceneReconciliation:\s*clone\(view\?\.campaignState\?\.sceneReconciliation\s*\|\|\s*null\)/);
assert.doesNotMatch(liveHarnessSource, /sceneReconciliation:\s*clone\([\s\S]*?runtimeTracking\?\.sceneReconciliation/);
assert.match(messageActionLiveRunnerSource, /const\s+reconciliation\s*=\s*view\?\.campaignState\?\.sceneReconciliation\s*\|\|\s*\{\}/);
assert.match(messageActionLiveRunnerSource, /recoveryCount:\s*count\(coreRecoveryJournal\)/);
assert.match(messageActionLiveRunnerSource, /recentRecoveryJournal:\s*clone\(coreRecoveryJournal\.slice\(-5\)\)/);
assert.doesNotMatch(messageActionLiveRunnerSource, /recoveryCount:\s*count\(tracking\.recoveryJournal\)/);
assert.doesNotMatch(messageActionLiveRunnerSource, /tracking\.sceneReconciliation|runtimeTracking\?\.sceneReconciliation/);
assert.doesNotMatch(architectureRedesignPlanSource, /top-level first with runtimeTracking only as a compatibility fallback/);
assert.doesNotMatch(architectureRedesignPlanSource, /service-created `runtimeTracking\.sceneReconciliation` snapshots now carry/);
assert.doesNotMatch(architectureRedesignPlanSource, /newest raw `runtimeTracking` row before consulting authoritative CORE projections/);
...
assert.doesNotMatch(liveSmokeSource, /Array\.isArray\(runtimeTracking\.ingressLedger\)/);
assert.doesNotMatch(liveSmokeSource, /Array\.isArray\(runtimeTracking\.responseLedger\)/);
assert.doesNotMatch(liveSmokeSource, /Array\.isArray\(view\?\.campaignState\?\.runtimeTracking\?\.recoveryJournal\)/);
