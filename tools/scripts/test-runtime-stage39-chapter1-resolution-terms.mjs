...
const opening = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage39.initial-response',
...
const threshold = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage39.boarding-threshold',
...
const execution = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage39.first-contact-execution',
...
const discovery = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage39.offsite-custody-cargo',
...
const pellTerms = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage39.pell-contact-terms',
...
const jointInspection = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage39.joint-inspection-release',
...
const cargoPulse = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage39.cargo-diagnostic-pulse',
...
const hardwareRecovery = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage39.hardware-recovery',
...
assert.equal(hardwareRecovery.campaignState.mission.activePhaseId, 'hardware-recovery-under-seal');
assert.deepEqual(hardwareRecovery.campaignState.mission.availableDecisionPointIds, ['decision.chapter-1-resolution-terms']);
...
const resolution = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage39.resolution-terms',
...
const committed = resolution.commit.campaignState;
const sourceOutcomeId = resolution.commit.turnPacket.outcomePacket.id;

assert.equal(resolution.preview.turnPacket.intentParse.primaryIntent, 'set-chapter1-resolution-terms');
assert.equal(resolution.preview.turnPacket.actionClassification.category, 'validWithinMissionBounds');
assert.equal(resolution.preview.turnPacket.authorityCapabilityCheck.result, 'authorizedAndFeasibleWithOperationalRisk');
assert.equal(resolution.commit.turnPacket.outcomePacket.resultBand, 'Success');
assert.equal(committed.mission.activePhaseId, 'chapter-1-resolution-terms');
assert.deepEqual(committed.mission.availableDecisionPointIds, ['decision.asterion-arrival-false-colors']);
assert.equal(committed.mission.knownFacts.includes('chapter-1.joint-incident-record-created'), true);
assert.equal(committed.mission.knownFacts.includes('chapter-1.cooperative-resolution-filed'), true);
assert.equal(committed.mission.knownFacts.includes('chapter-1.starfleet-authentication-failure-acknowledged'), true);
...
assert.equal(flagValue(committed, 'chapter-1.resolution-path'), 'cooperative');
assert.equal(flagValue(committed, 'chapter-1.incident-record-status'), 'joint-record-created');
assert.equal(flagValue(committed, 'chapter-1.ivers-trust'), 'trusts-player');
assert.equal(flagValue(committed, 'chapter-1.pell-status'), 'witness-recruited');
assert.equal(flagValue(committed, 'chapter-1.compact-investigation-access'), 'joint-access');
assert.equal(flagValue(committed, 'chapter-1.authentication-failure-posture'), 'publicly-acknowledged');
assert.equal(flagValue(committed, 'chapter-1.parnell-technical-debt'), 'documented-contained');
assert.equal(flagValue(committed, 'chapter-1.compact-posture'), 'joint-record-access');
assert.equal(flagValue(committed, 'chapter-1.joint-inspection-status'), 'joint-incident-record-created');
assert.equal(flagValue(committed, 'chapter-1.pell-contact'), 'witness-cooperation-secured');
assert.equal(flagValue(committed, 'chapter-1.cargo-recovery-route'), 'resolved-under-joint-record');
...
assert.equal(frontById(committed, 'front.chapter-1.rescue')?.status, 'crews-rescued-recorded');
assert.equal(frontById(committed, 'front.chapter-1.medical-quarantine')?.status, 'shelter-followup-documented');
assert.equal(frontById(committed, 'front.chapter-1.security-exposure')?.status, 'contained');
assert.equal(frontById(committed, 'front.chapter-1.evidence-custody')?.status, 'joint-record-created');
assert.equal(frontById(committed, 'front.chapter-1.regional-diplomacy')?.status, 'regional-trust-improved');
assert.equal(actorPosture(committed, 'relief-convoy-twelve')?.posture, 'ivers-trust-secured');
assert.equal(actorPosture(committed, 'uss-breckinridge')?.posture, 'cooperative-resolution-filed');
assert.equal(actorPosture(committed, 'compact-recovery-team')?.posture, 'contained-by-joint-record');
...
for (const frontId of ['front.chapter-1.rescue', 'front.chapter-1.medical-quarantine', 'front.chapter-1.security-exposure', 'front.chapter-1.evidence-custody', 'front.chapter-1.regional-diplomacy']) {
...
assert.deepEqual(restored.fronts || [], hardwareRecovery.campaignState.fronts || [], 'delete restores pre-resolution fronts');
assert.deepEqual(restored.actors?.postures || [], hardwareRecovery.campaignState.actors?.postures || [], 'delete restores pre-resolution actor postures');
assert.equal(restored.mission.activePhaseId, 'hardware-recovery-under-seal');
assert.deepEqual(restored.mission.availableDecisionPointIds, ['decision.chapter-1-resolution-terms']);

assertHiddenTermsAbsent(resolution.preview.turnPacket.outcomePacket);
assertHiddenTermsAbsent(resolution.preview.turnPacket.narratorPacket);
assertHiddenTermsAbsent(resolution.preview.turnPacket.commandLogPacket);
assertHiddenTermsAbsent(resolution.commit.turnPacket.outcomePacket);
assertHiddenTermsAbsent(resolution.commit.turnPacket.narratorPacket);
assertHiddenTermsAbsent(resolution.commit.turnPacket.commandLogPacket);
...
console.log('Stage 39 Chapter 1 resolution tests passed: cooperative incident record, witness trust, Compact access, authentication accountability, fronts, actors, save/load, rollback, and hidden-truth safety');
