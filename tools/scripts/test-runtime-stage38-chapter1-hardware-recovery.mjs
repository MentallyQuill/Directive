...
const opening = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage38.initial-response',
...
const threshold = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage38.boarding-threshold',
...
const execution = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage38.first-contact-execution',
...
const discovery = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage38.offsite-custody-cargo',
...
const pellTerms = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage38.pell-contact-terms',
...
const jointInspection = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage38.joint-inspection-release',
...
const cargoPulse = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage38.cargo-diagnostic-pulse',
...
assert.equal(cargoPulse.campaignState.mission.activePhaseId, 'cargo-diagnostic-pulse');
assert.deepEqual(cargoPulse.campaignState.mission.availableDecisionPointIds, ['decision.hardware-recovery-under-seal']);
...
const hardwareRecovery = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage38.hardware-recovery',
...
const committed = hardwareRecovery.commit.campaignState;
const sourceOutcomeId = hardwareRecovery.commit.turnPacket.outcomePacket.id;

assert.equal(hardwareRecovery.preview.turnPacket.intentParse.primaryIntent, 'recover-hardware-under-seal');
assert.equal(hardwareRecovery.preview.turnPacket.actionClassification.category, 'validWithinMissionBounds');
assert.equal(hardwareRecovery.preview.turnPacket.authorityCapabilityCheck.result, 'authorizedAndFeasibleWithOperationalRisk');
assert.equal(hardwareRecovery.commit.turnPacket.outcomePacket.resultBand, 'Success');
assert.equal(committed.mission.activePhaseId, 'hardware-recovery-under-seal');
assert.deepEqual(committed.mission.availableDecisionPointIds, ['decision.chapter-1-resolution-terms']);
assert.equal(committed.mission.knownFacts.includes('chapter-1.emergency-hardware-recovered-under-seal'), true);
assert.equal(committed.mission.knownFacts.includes('chapter-1.recovery-timing-trace-preserved'), true);
...
assert.equal(flagValue(committed, 'chapter-1.recovered-hardware-status'), 'recovered-under-joint-seal');
assert.equal(flagValue(committed, 'chapter-1.cargo-location'), 'recovered-under-seal');
assert.equal(flagValue(committed, 'chapter-1.cargo-recovery-route'), 'hardware-recovered-under-seal');
...
assert.equal(flagValue(committed, 'chapter-1.missing-module-lead'), 'recovered-intact');
...
assert.equal(frontById(committed, 'front.chapter-1.evidence-custody')?.status, 'hardware-recovered-under-seal');
assert.equal(frontById(committed, 'front.chapter-1.regional-diplomacy')?.status, 'joint-custody-preserved');
assert.equal(actorPosture(committed, 'relief-convoy-twelve')?.posture, 'ivers-supervised-release-secured');
assert.equal(actorPosture(committed, 'uss-breckinridge')?.posture, 'hardware-recovered-under-seal');
assert.equal(actorPosture(committed, 'compact-recovery-team')?.posture, 'joint-custody-witness');
...
assert.deepEqual(restored.fronts || [], cargoPulse.campaignState.fronts || [], 'delete restores pre-hardware-recovery fronts');
assert.deepEqual(restored.actors?.postures || [], cargoPulse.campaignState.actors?.postures || [], 'delete restores pre-hardware-recovery actor postures');
assert.equal(restored.mission.activePhaseId, 'cargo-diagnostic-pulse');
assert.deepEqual(restored.mission.availableDecisionPointIds, ['decision.hardware-recovery-under-seal']);

assertHiddenTermsAbsent(hardwareRecovery.preview.turnPacket.outcomePacket);
assertHiddenTermsAbsent(hardwareRecovery.preview.turnPacket.narratorPacket);
assertHiddenTermsAbsent(hardwareRecovery.preview.turnPacket.commandLogPacket);
assertHiddenTermsAbsent(hardwareRecovery.commit.turnPacket.outcomePacket);
assertHiddenTermsAbsent(hardwareRecovery.commit.turnPacket.narratorPacket);
assertHiddenTermsAbsent(hardwareRecovery.commit.turnPacket.commandLogPacket);
...
console.log('Stage 38 Chapter 1 hardware recovery tests passed: hardware under joint seal, timing trace, fronts, actors, save/load, rollback, and hidden-truth safety');
