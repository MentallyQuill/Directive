...
assert.equal(findControl(panel, 'identity.name').value, 'Sam Vickers', 'Provider retry should auto-apply a usable empty-section draft');
...
assert.equal(creatorDraftCalls.length, 2, 'Identity wand should retry after an unusable provider draft');
assert.equal(creatorDraftCalls[0].request.mode, 'create');
assert.equal(creatorDraftCalls[1].request.mode, 'create');
...
assert.equal(creatorDraftCalls.length, 3, 'Partial identity wand should make another provider request');
assert.equal(creatorDraftCalls[2].request.mode, 'refine');
...
setRuntimeControl(panel, 'settings.maxTurnSaveHistory', '6');
assert.equal(runtimeApplyButton.disabled, false, 'Runtime Apply should activate for runtime history changes');
await runtimeApplyButton.click();
assert.equal(findControl(panel, 'settings.maxTurnSaveHistory').value, '6');
assert.equal(findControl(panel, 'settings.autosaveEveryMessages').value, '1');
updatedSaves = await listCampaignSaves(adapter);
...
assert.equal(Boolean(runtimeSettingsSaveIndexEntry.v2ManifestRef?.logicalKey), true, 'Runtime settings Apply must attach a v2 runtime manifest ref.');
assert.equal(Boolean(runtimeSettingsSaveIndexEntry.v2RuntimePersistedAt), true, 'Runtime settings Apply must attach a v2 runtime persistence timestamp.');
...
assert.equal(settledRuntimeSettingsPayload.payload.campaignState.settings.maxTurnSaveHistory, 6, 'Settle Active State checkpoint must capture runtime-current history setting.');
assert.equal(settledRuntimeSettingsPayload.payload.campaignState.settings.autosaveEveryMessages, 1, 'Settle Active State checkpoint must capture runtime-current autosave setting.');
const directSettleResult = await app.settleActiveState();
assert.equal(directSettleResult.save.kind, 'directive.campaignSave', 'Direct Settle API must return the v1 checkpoint save record.');
assert.equal(directSettleResult.save.schemaVersion, 1, 'Direct Settle API must remain on the v1 checkpoint schema.');
assert.equal(directSettleResult.save.id, settledRuntimeSettingsSave.id, 'Direct Settle API must keep the active save slot.');
assert.equal(directSettleResult.save.storageFormat, undefined, 'Direct Settle API must not masquerade as a v2 runtime persist.');
assert.equal(directSettleResult.save.runtimeStorageFormat, undefined, 'Direct Settle API must not return v2 runtime marker fields.');
assert.equal(directSettleResult.save.v2ManifestRef, undefined, 'Direct Settle API must not return a v2 runtime manifest ref.');
indexesAfterRuntimeSettings = await getDirectiveStorageIndexes(adapter);
runtimeSettingsSaveIndexEntry = indexesAfterRuntimeSettings.saveIndex.saves[settledRuntimeSettingsSave.id];
