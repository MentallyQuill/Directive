...
const opening = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage36.initial-response',
...
const threshold = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage36.boarding-threshold',
...
const execution = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage36.first-contact-execution',
...
const discovery = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage36.offsite-custody-cargo',
...
const pellTerms = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage36.pell-contact-terms',
...
assert.equal(pellTerms.campaignState.mission.activePhaseId, 'pell-contact-terms');
assert.deepEqual(pellTerms.campaignState.mission.availableDecisionPointIds, ['decision.joint-inspection-release-cargo']);
...
const jointInspection = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage36.joint-inspection-release',
...
const committed = jointInspection.commit.campaignState;
const sourceOutcomeId = jointInspection.commit.turnPacket.outcomePacket.id;

assert.equal(jointInspection.preview.turnPacket.intentParse.primaryIntent, 'execute-joint-inspection-release');
assert.equal(jointInspection.preview.turnPacket.actionClassification.category, 'validWithinMissionBounds');
assert.equal(jointInspection.preview.turnPacket.authorityCapabilityCheck.result, 'authorizedAndFeasibleWithOperationalRisk');
assert.equal(jointInspection.commit.turnPacket.outcomePacket.resultBand, 'Success');
assert.equal(committed.mission.activePhaseId, 'joint-inspection-release-cargo');
assert.deepEqual(committed.mission.availableDecisionPointIds, ['decision.cargo-diagnostic-pulse']);
assert.equal(committed.mission.knownFacts.includes('chapter-1.ivers-supervised-statement'), true);
assert.equal(committed.mission.knownFacts.includes('chapter-1.joint-inspection-record-opened'), true);
...
assert.equal(flagValue(committed, 'chapter-1.cargo-recovery-route'), 'joint-inspection-in-progress');
...
assert.equal(frontById(committed, 'front.chapter-1.evidence-custody')?.status, 'joint-inspection-record-open');
assert.equal(frontById(committed, 'front.chapter-1.regional-diplomacy')?.status, 'joint-inspection-active');
assert.equal(actorPosture(committed, 'relief-convoy-twelve')?.posture, 'ivers-supervised-release-secured');
assert.equal(actorPosture(committed, 'uss-breckinridge')?.posture, 'joint-inspection-executing');
assert.equal(actorPosture(committed, 'compact-recovery-team')?.posture, 'lawful-exit-in-progress');
...
assert.deepEqual(restored.fronts || [], pellTerms.campaignState.fronts || [], 'delete restores pre-joint-inspection fronts');
assert.deepEqual(restored.actors?.postures || [], pellTerms.campaignState.actors?.postures || [], 'delete restores pre-joint-inspection actor postures');
assert.equal(restored.mission.activePhaseId, 'pell-contact-terms');
assert.deepEqual(restored.mission.availableDecisionPointIds, ['decision.joint-inspection-release-cargo']);

assertHiddenTermsAbsent(jointInspection.preview.turnPacket.outcomePacket);
assertHiddenTermsAbsent(jointInspection.preview.turnPacket.narratorPacket);
assertHiddenTermsAbsent(jointInspection.preview.turnPacket.commandLogPacket);
assertHiddenTermsAbsent(jointInspection.commit.turnPacket.outcomePacket);
assertHiddenTermsAbsent(jointInspection.commit.turnPacket.narratorPacket);
assertHiddenTermsAbsent(jointInspection.commit.turnPacket.commandLogPacket);
...
console.log('Stage 36 Chapter 1 joint inspection tests passed: shared record, supervised Ivers release, cargo evidence route, fronts, actors, save/load, rollback, and hidden-truth safety');
