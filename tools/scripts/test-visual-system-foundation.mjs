...
assert.match(css, /\.directive-log-status-grid \.directive-log-status-block\s*\{[\s\S]*?grid-template-columns:\s*minmax\(0,\s*1fr\)\s*!important;[\s\S]*?grid-template-rows:\s*auto auto\s*!important;[\s\S]*?align-items:\s*start\s*!important;/, 'Log summary status blocks should not reserve an empty icon column that clips full labels');
assert.match(css, /\.directive-log-status-block \.directive-lcars-status-label,[\s\S]*?\.directive-log-status-block \.directive-lcars-status-value\s*\{[\s\S]*?grid-column:\s*1\s*!important;/, 'Log summary status labels and values should share the full tile width');
