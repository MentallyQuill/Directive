...
assert.match(css, /\.directive-command-spine-shell \.directive-ship-hero\s*\{[\s\S]*?grid-template-columns:\s*minmax\(0,\s*1fr\)\s+minmax\(170px,\s*0\.38fr\)\s*!important;/, 'command-spine Ship hero should keep the ship picture on the right at drawer width');
assert.match(css, /@container\s*\(max-width:\s*560px\)\s*\{\s*\.directive-command-spine-shell \.directive-ship-hero\s*\{[\s\S]*?grid-template-columns:\s*minmax\(0,\s*1fr\)\s*!important;/, 'command-spine Ship hero should stack only in very narrow drawers');
...
assert.match(css, /\.directive-command-spine-shell \.directive-spine-route-selected:hover,[\s\S]*?\.directive-command-spine-shell \.directive-spine-route-selected:focus-visible\s*\{[\s\S]*?background:\s*linear-gradient\(155deg,\s*var\(--directive-route-accent\),\s*var\(--directive-route-accent-end\)\)\s*!important;[\s\S]*?outline:\s*2px\s+solid\s+var\(--directive-focus,\s*#ffe58f\);/, 'selected shelf route hover should keep the selected fill while retaining the outline');
assert.match(css, /\.directive-command-spine-shell \.directive-spine-route-selected:hover \.directive-spine-route-index,[\s\S]*?\.directive-command-spine-shell \.directive-spine-route-selected:focus-visible \.directive-spine-route-detail\s*\{[\s\S]*?color:\s*var\(--directive-active-route-ink\)\s*!important;/, 'selected shelf route hover should not invert child text or icon colors independently');
