...
export const SOAK_TURN_SCRIPT = Object.freeze([
  ...
  intent(7, 'push toward a risky close approach with explicit limits: "Conn, target a passive intercept geometry that narrows time to the sector boundary by no more than twelve minutes; method is a warp-three trim inside the filed transit corridor, passive sensors only, weapons cold, and no DMZ protocol breach. Operations logs the objective pressure, open assignments, and risk note before execution; Captain Whitaker keeps final approval." Then verify Mission drawer pressure and objective updates', 'mission-drawer'),
  intent(8, 'accept Doctor Sato and Bronn\'s readiness warning with explicit mitigation: "Operations, target shield-generator standby validation; method is warm-standby shields readiness checks only, no shield raise, no active emissions. Department heads, target fatigue control for the next six hours; method is mandatory four-hour rotations and a six-hour command review, with exceptions reported to Arlen and Captain Whitaker." Then verify relationship consequences are tracked behind the curtain and projected safely when appropriate', 'relationship-delta'),
...
function messageProseForTurn(entry = {}) {
  ...
  if (turn === 7) {
    return `${commandRail} and turns the risky approach into a constrained order rather than an open-ended push. "Conn, target a passive intercept geometry that narrows our time to the sector boundary by no more than twelve minutes. Method: warp-three trim inside the filed transit corridor, passive sensors only, weapons cold, and no DMZ protocol breach. Operations, log the objective pressure, open assignments, and risk note before execution. Captain Whitaker retains final approval." Arlen waits for the crew and the Mission drawer record to reflect the actual outcome.`;
  }
  if (turn === 8) {
    return `${commandRail} and accepts Sato and Bronn's warning by naming the mitigation instead of waving it aside. "Operations, target shield-generator standby validation. Method: warm-standby shields readiness checks only, no shield raise and no active emissions. Department heads, target fatigue control for the next six hours. Method: mandatory four-hour rotations and a six-hour command review, with exceptions reported to Arlen and Captain Whitaker." Arlen leaves the officers room to react in their own voices while Directive tracks pressure and relationship consequences behind the curtain.`;
  }
