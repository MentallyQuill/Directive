...
assert.equal(intercepted.abortDefaultGeneration, true, 'a canceled mandatory analysis must remain behind explicit Retry');
assert.equal(intercepted.settlementError.reasonCode, 'provider-aborted');
assert.equal(generation.calls().filter(call => call.role === 'acceptedPairMissionEvidence').length, 1, 'correction recovery must not retry a provider automatically');
const retry = app.retryPendingAcceptedPairSettlement();
await new Promise(resolve => setTimeout(resolve, 0));
release();
assert.equal((await retry).ok, true, 'explicit Retry may run a fresh complete analysis');
