...
`check-sillytavern-multi-user-soak-readiness.mjs` is the live preflight for parallel soak workers. Dry-run mode validates the configured user list and Playwright availability without contacting the host. Live mode requires `SILLYTAVERN_BASE_URL`, at least two users in `DIRECTIVE_SOAK_ST_USERS`, and `--live`; it opens one Playwright browser context per ST user, logs in when required, writes a unique Directive `/user/files` probe per user, verifies each user sees only its own probe, deletes the probes, and writes artifacts under the soak run folder. Use this before running parallel workers or before trusting per-user extension patch lanes.
