...
const opening = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage35.initial-response',
...
const threshold = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage35.boarding-threshold',
...
const execution = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage35.first-contact-execution',
...
const discovery = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage35.offsite-custody-cargo',
...
assert.equal(discovery.campaignState.mission.activePhaseId, 'offsite-custody-cargo-leads');
assert.deepEqual(discovery.campaignState.mission.availableDecisionPointIds, ['decision.pell-contact-terms']);
...
const pellTerms = commitInput({
  ...
  projection,
  crewDataset,
  turnId: 'turn.stage35.pell-contact-terms',
...
const committed = pellTerms.commit.campaignState;
const sourceOutcomeId = pellTerms.commit.turnPacket.outcomePacket.id;

assert.equal(pellTerms.preview.turnPacket.intentParse.primaryIntent, 'set-pell-contact-terms');
assert.equal(pellTerms.preview.turnPacket.actionClassification.category, 'validWithinMissionBounds');
assert.equal(pellTerms.preview.turnPacket.authorityCapabilityCheck.result, 'authorizedAndFeasibleWithOperationalRisk');
assert.equal(pellTerms.commit.turnPacket.outcomePacket.resultBand, 'Success');
assert.equal(committed.mission.activePhaseId, 'pell-contact-terms');
assert.deepEqual(committed.mission.availableDecisionPointIds, ['decision.joint-inspection-release-cargo']);
assert.equal(committed.mission.knownFacts.includes('chapter-1.pell-separate-warning'), true);
assert.equal(committed.mission.knownFacts.includes('chapter-1.emergency-transponder-hardware-manifest'), true);
...
assert.equal(flagValue(committed, 'chapter-1.pell-contact'), 'joint-inspection-open');
assert.equal(flagValue(committed, 'chapter-1.ivers-status'), 'release-negotiation-open');
assert.equal(flagValue(committed, 'chapter-1.cargo-recovery-route'), 'joint-inspection-undertaking');
...
assert.equal(frontById(committed, 'front.chapter-1.evidence-custody')?.status, 'cargo-undertaking-open');
assert.equal(frontById(committed, 'front.chapter-1.regional-diplomacy')?.status, 'joint-inspection-open');
assert.equal(actorPosture(committed, 'relief-convoy-twelve')?.posture, 'officers-release-negotiation-open');
assert.equal(actorPosture(committed, 'uss-breckinridge')?.posture, 'joint-inspection-terms-open');
assert.equal(actorPosture(committed, 'compact-recovery-team')?.posture, 'lawful-exit-visible');
...
assert.deepEqual(restored.fronts || [], discovery.campaignState.fronts || [], 'delete restores pre-Pell-contact fronts');
assert.deepEqual(restored.actors?.postures || [], discovery.campaignState.actors?.postures || [], 'delete restores pre-Pell-contact actor postures');
assert.equal(restored.mission.activePhaseId, 'offsite-custody-cargo-leads');
assert.deepEqual(restored.mission.availableDecisionPointIds, ['decision.pell-contact-terms']);

assertHiddenTermsAbsent(pellTerms.preview.turnPacket.outcomePacket);
assertHiddenTermsAbsent(pellTerms.preview.turnPacket.narratorPacket);
assertHiddenTermsAbsent(pellTerms.preview.turnPacket.commandLogPacket);
assertHiddenTermsAbsent(pellTerms.commit.turnPacket.outcomePacket);
assertHiddenTermsAbsent(pellTerms.commit.turnPacket.narratorPacket);
assertHiddenTermsAbsent(pellTerms.commit.turnPacket.commandLogPacket);
...
console.log('Stage 35 Chapter 1 Pell contact tests passed: contact terms, Ivers release route, cargo undertaking, fronts, actors, save/load, rollback, and hidden-truth safety');
