...
const d=client.generate('storyDirectionAnalyst',{prompt:'direction',parameters:{max_tokens:2048}},{signal:controller.signal,allowVisibleOutputRetry:false});
const c=client.generate('continuityAnalyst',{prompt:'continuity',parameters:{max_tokens:6144}},{signal:controller.signal,allowVisibleOutputRetry:false});
const e=client.generate('episodeEvaluator',{prompt:'episode',parameters:{max_tokens:2048}},{signal:controller.signal,allowVisibleOutputRetry:false});
assert.equal(requests.length,4,'all four native request boundaries start before any completes');
...
pending[1]({text:'{}'}); await d; assert.equal(isDirectiveOwnedHostGeneration(),true);
pending[2]({text:'{}'}); await c; assert.equal(isDirectiveOwnedHostGeneration(),true);
pending[3]({text:'{}'}); await e; assert.equal(isDirectiveOwnedHostGeneration(),false);
let attempts=0;
