...
assert.equal(registry.get('defineSelection').providerKind, 'utility');
assert.equal(registry.get('defineSelection').structuredOutput, true);
assert.equal(registry.get('defineSelection').mayProposeState, false);
assert.equal(registry.get('defineSelection').mayInjectPrompt, false);
assert.equal(registry.get('defineSelection').fallback, 'deterministic');
