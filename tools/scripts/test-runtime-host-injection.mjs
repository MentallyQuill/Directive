...
assert.equal(factualReview.modelCallDelta, 0, 'review-only generation without CORE target must not grow durable model-call diagnostics');
...
assert.equal(factualReview.requestId, 'fact-model-review-host-injection');
assert.equal(factualReview.modelCall.metadata, undefined, 'model-call journal rows must not persist arbitrary request metadata');
...
assert.equal(storyQualityReview.modelCallDelta, 0, 'story-quality generation without CORE target must not grow durable model-call diagnostics');
...
assert.equal(storyQualityReview.requestId, 'story-quality-review-host-injection');
assert.equal(storyQualityReview.modelCall.metadata, undefined, 'model-call journal rows must not persist arbitrary request metadata');
