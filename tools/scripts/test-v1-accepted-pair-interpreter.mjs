...
assert.match(prompt.systemPrompt, /"time":\{"decision":"unchanged","basis":"noPassage","elapsedSeconds":0/);
assert.match(prompt.systemPrompt, /Implicit-action time example only:.*"basis":"implicitAction".*"sourceSlot":"currentPlayer".*"evidenceQuote":/);
assert.match(prompt.systemPrompt, /new time evidence and scenePacing participation.*only sourcePair\.previousAssistant\.text or sourcePair\.currentPlayer\.text/i);
assert.match(prompt.systemPrompt, /alreadyCountedPlayer.*currentScene\.participation.*historical context.*never.*new evidence/i);
assert.match(prompt.systemPrompt, /do not add a closing quotation mark unless.*exact excerpt boundary/i);
