...
export function extractProviderResponseReasoning(value = '') {
  ...
  const details = message.reasoning_details || message.reasoningDetails || choice?.message?.reasoning_details || value.reasoning_details || value.reasoningDetails;
...
export function getProviderResponseFailure(value = '', options = {}) {
  ...
  if (reasoning.trim()) {
    ...
    const retryText = retried ? ' Retried with final-only visible-output instructions but still received no visible content.' : '';
    return {
      ...description,
      code: PROVIDER_RESPONSE_ERROR_CODES.REASONING_ONLY,
      providerTitle,
      reasoningPreview: reasoning.slice(0, 300),
      message: `${providerTitle} provider returned reasoning-only output with empty visible content.${retryText}`
    };
  }
