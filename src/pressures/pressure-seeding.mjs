...
function preludePressureSeeds({ campaignState, outcomePacket, intentParse }) {
  ...
  if (includesAny(shipState, ['limitation', 'incomplete', 'workaround', 'untested', 'concealed-risk'])) {
    seeds.push({
      ...
      playerSummary: 'Imani still has unresolved Breckenridge technical debt to make visible before it becomes routine.',
...
function chapter1PressureSeeds({ campaignState, outcomePacket, intentParse }) {
  ...
  seeds.push({
    ...
    playerSummary: `The first response to Relief Convoy Twelve now shapes how nearby authorities read the Breckenridge's priorities: ${posture}.`,
...
function chapter2PressureSeeds({ campaignState, outcomePacket, intentParse }) {
  ...
  return [{
    ...
    playerSummary: 'Priya needs a calm, accountable coordination channel after the False Colors charter so Kessler, station operators, Compact observers, civilian couriers, and Breckenridge Ops can share records while the weak Hecate trace stays in a correlation queue, not a pursuit target.',
    directorSummary: 'The joint investigation charter creates Open Orders pressure for Priya to convert public legitimacy, audit-record protection, and temporary Breckenridge presence into accountable regional coordination while forensic specialists travel.',
    urgencyBand: 'medium',
    escalationBand: 'signal',
    linkedCrewIds: ['priya-nayar'],
