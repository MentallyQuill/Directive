...
export const CHARACTER_SCENE_ANALYSIS_POLICY = [
  ...
  'Choose the outgoing route to match the evidenced response channel: when the source requests a private visible or typed response and establishes that the recipient can see it, select observed for that recipient. An observed route can carry externally observable action or speech; a heard/read-only route cannot carry action. Do not infer visual access from a remote audio connection or add an unsupported route.',
