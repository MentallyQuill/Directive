...
function hasAwardedDecision(campaignState, decisionId) {
  const bearing = campaignState?.commandBearing || {};
  const inspiration = (bearing.tracks?.inspiration || bearing.inspiration)?.awardedDecisionIds || [];
  const resolve = (bearing.tracks?.resolve || bearing.resolve)?.awardedDecisionIds || [];
