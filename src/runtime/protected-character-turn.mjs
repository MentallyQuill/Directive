...
import { createCharacterSceneNarrator } from '../narration/character-scene-narrator.mjs';
...
export async function prepareProtectedCharacterTurn({ generation, campaignState, crewDataset, messages,
  ...
  hostMessageId = null, requireEmpty = false, signal, settings, pacing = null, limits = {}, onPhase } = {}) {
  ...
  try {
    ...
    const approved = await pipeline.generate({ flight, scenePacket, budget, signal, settings, pacing, onPhase });
    let disposed = false;
    const dispose = () => { if (!disposed) { disposed = true; flight.dispose(); budget.dispose(); } };
    return {
      publicationId, get attempts() { return budget.used; }, dispose,
      async publish(publisher) {
        assertGenerationActive(signal);
        if (disposed || !guard.isCurrent()) throw Object.assign(new Error('Protected turn is stale.'), { code: 'DIRECTIVE_CHARACTER_SCENE_STALE' });
        try {
          const result = await publisher({ publicationId, text: approved.candidate.text, expectedBinding, hostMessageId, requireEmpty, signal,
            assertCurrent: input => {
              if (!guard.assertPublication(input)) return false;
              flight.assertCurrent(approved.draft.flightDigest);
              return true;
            },
            createMetadata: source => createCharacterScenePublicationMetadata({ approved, scenePacket, pacing, publicationId, source }),
          });
