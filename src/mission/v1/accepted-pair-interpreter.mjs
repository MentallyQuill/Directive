...
export function createMissionAcceptedPairInterpretationPrompt({
    ...
    const systemPrompt = [
        ...(evidenceCatalog ? [EVIDENCE_REFERENCE_INSTRUCTIONS,
            'Passage IDs are opaque: copy the exact complete ID from evidencePassages. Never reconstruct, shorten, or repair an ID.',
            `Use a short rationale phrase for time.reason; aim for at most ${Math.max(1, Math.min(60, Math.floor(limits.timeReasonCharacters / 2)))} characters. Do not recap the exchange or include duration arithmetic. The schema hard limit still applies.`,
        ] : []),
        ...
        ...(candidatePacket.scenePacing ? [
            ...
            'A greeting, broad order, general information request, or NPC monologue is not completed participation. Cite contiguous exact quotes from BOTH currentPlayer and previousAssistant for each requirement actually engaged. Do not invent agreement or treat narrated player conduct as player participation.',
            'requirement is a zero-based index in the selected objective scenePacing.requirements array (two requirements use 0 and 1). If either speaker lacks a supporting quote, omit the participation item; never supply an empty quote.',
            ...
            'Scope unresolved to material questions or objections about the selected objective; a next objective, meeting, or activity does not keep this one unresolved.',
            'When the authored objective is to establish an agreement, delegation, or procedure, distinguish agreeing its owners and limits from carrying out its future tasks; pending execution alone is not unresolved unless the authored objective requires that execution.',
            'After the required discussion, a player request to enact or confirm the concrete final decision or authority transfer may be resolve; the requested outcome itself is not an unanswered prerequisite, and a general request for information remains continue.',
            'When the player explicitly ends or leaves the current interaction, use leave with that exact player quote even if their next activity has open questions; leaving alone never proves objective completion.',
            `For leave, delegate, skip, or resolve, ${evidenceCatalog ? 'intentPassageId must select a CURRENT PLAYER passage' : 'intentQuote must quote the CURRENT PLAYER'}. Mentioning a future departure or discussing delegation is not enacting it. Requests for information are not requests to fast-forward. Never move the story because enough turns have elapsed.`,
            `Set ${evidenceCatalog ? 'missionDeparturePassageId' : 'missionDepartureQuote'} only when the CURRENT PLAYER explicitly chooses to finish this mission, conclude the interval/campaign, or proceed to the next mission. Leaving a room, concluding one conversation, delegating an objective, and asking about the next assignment do NOT authorize mission departure. Otherwise return ${evidenceCatalog ? 'null missionDeparturePassageId' : 'an empty missionDepartureQuote'}.`,
        ] : []),
        ...
        'For new time evidence and scenePacing participation, quote only sourcePair.previousAssistant.text or sourcePair.currentPlayer.text. time.alreadyCountedPlayer and scenePacing.currentScene.participation are historical context only, never new evidence; do not copy their quotations into the output.',
        ...
        `The complete output schema below applies in Prompt JSON mode as well as native schema mode. Omit claim value unless that candidate permits it. People observations use type, personRef (or localRef for introductions), ${evidenceCatalog ? 'evidencePassageId' : 'sourceSlot, and evidenceQuote'}; do not invent alternative field names. Keep time.reason within 180 characters.`,
