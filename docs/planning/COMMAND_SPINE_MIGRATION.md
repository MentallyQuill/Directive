...
Phase 1 is implemented for the SillyTavern runtime shell.
...
The Lumiverse frontend still mounts `directive-compact-shell.js` during this phase. It continues to use the prior compact bottom-navigation shell until its host bridge is migrated to the same command-spine contract.
...
- Migrate the Lumiverse frontend from `directive-compact-shell.js` to the command-spine shell after its frontend bridge can persist host-local layout state.
