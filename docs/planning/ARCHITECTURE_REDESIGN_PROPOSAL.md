...
July 1 bridge tightening moved two remaining compatibility surfaces closer to CORE authority. Runtime sidecar progress/freshness now counts CORE `sidecarDiagnostics` instead of relying only on old `runtimeTracking.sidecarJournal` growth, which matters because accepted FORGE-settled sidecars intentionally no longer write old applied journals. Accepted regular sidecar prompt flushes also receive a compact `directive.coreAcceptedSidecarBatchProjection.v1` containing the CORE/FORGE transaction, batch, idempotency, accepted-hash, worker, dirty-domain, source Frame, and settlement receipt evidence. The scheduler still passes the temporary compatibility `campaignState` for now, but FORGE/LENS no longer has to infer accepted-batch identity solely from that old projection.

June 30 response-recovery closeout: `chat-turn-orchestrator.mjs` no longer lets a CORE-backed Directive response post failure quietly become an old-ledger-only retry case. If a turn has a CORE transaction and REPAIR/CORE cannot record `responseRetryRequired`, the turn surfaces `DIRECTIVE_CORE_RESPONSE_RECOVERY_NOT_RECORDED` and preserves only a generic processing-recovery trail rather than creating an independent `hostResponsePostFailure` authority in the old recovery journal. When CORE does accept the recovery, the old journal carries compact projection evidence (`coreTransactionId`, `coreRecovery`, `repairDecision`) for UI/runtime bridge compatibility. This narrows the remaining REPAIR gap to generic retry/rerun/rollback actuation and SRE source review, not response-recovery policy creation.

July 1 response-retry actuation tightening: `retryCommittedResponse(...)` now requires REPAIR authorization for every `hostResponsePostFailure` retry attempt, including old recovery rows that lack a CORE transaction. A no-CORE old-ledger retry returns `response-retry-not-authorized` with the compact REPAIR decision and does not dispatch a response, resolve the recovery, or persist campaign state. This removes one independent old-ledger retry authority. It does not yet close broader outcome rerun, provider-failure retry/reobserve, rollback execution, or final SRE review-worker ownership.
...
2. Completed bridge slice: parse and validate worker results before mutation, then aggregate accepted non-conflicting operations into one non-mutating compatibility projection preview, one CORE background batch settlement, post-settlement compatibility persistence, and one prompt dirty/rebuild decision.
...
14. Completed bridge slice: prove accepted regular sidecar bridge consistency with FORGE prepare, old projection no-persist validation, final CORE settlement, and post-CORE compatibility persist ordering. The bridge no longer persists the old compatibility projection before durable CORE/FORGE evidence exists; post-CORE compatibility-persist failure becomes compact warning evidence instead of creating a pre-CORE split-brain state. Ordinary accepted-batch prompt sync may still run from the in-memory projected state after durable CORE settlement, while Command Bearing review prompt sync remains blocked on its separate review compatibility persistence gate. Durable accepted settlements now validate the real CORE/FORGE receipt shape against transaction id, batch id, idempotency key, and `acceptedBatchHash`; FORGE computes worker-result hashes when available, rejects supplied/computed hash mismatches, and rejects effectful cached replay without matching accepted-batch evidence; CORE returned/hydrated `backgroundBatches[]` preserve compact batch/effect refs needed by scene seals, pressure digests, recall entries, open-world boundaries, and Command Bearing review `reviewHash` checks without raw provider/review text. The former scheduler-built direct CORE fallback was hardened during this proof and removed in the later no-FORGE cutover.
15. Completed bridge slice: move accepted regular sidecar prompt synchronization behind a FORGE/LENS-owned post-settlement prompt adapter. The scheduler still persists the temporary compatibility projection after FORGE/CORE settlement, but accepted sidecars no longer use old aggregate mutation before settlement; they use `stateDeltaGateway.validateOperations(...)` as a non-mutating projection preview and assign/persist that projection only after final settlement succeeds. After that persist succeeds it asks FORGE to flush the accepted-batch prompt through a runtime-owned `acceptedBatchPromptFlusher` that delegates to the real `synchronizeActivePrompt(...)` path, adopts returned campaign state, and preserves stable dirty-domain/idempotency evidence. The accepted prompt key includes both provider request idempotency evidence and accepted worker-result hash evidence so LENS does not suppress distinct accepted batches or same-source request retries. Rejected bridge outcomes, recovery-required results, post-settlement warning results, and unsafe FORGE statuses do not enter the completed provider-batch replay cache. If FORGE participates in the accepted-batch lifecycle, a missing or falsey FORGE/LENS accepted prompt helper now records compact `DIRECTIVE_SIDECAR_POST_SETTLEMENT_LENS_FLUSH_UNAVAILABLE` warning evidence instead of falling back to scheduler-owned `syncPromptContext(...)`; replay carries the same warning so it cannot become a clean provider-cache barrier. No-FORGE accepted batches now fail closed before old mutation. Post-CORE compatibility-persist failure no longer blocks ordinary accepted-batch prompt synchronization, but Command Bearing review prompt synchronization still requires its separate post-review compatibility persistence gate.
16. Completed bridge slice: move Command Bearing closure-review prompt synchronization behind a FORGE/LENS-owned review prompt adapter. Review prompt flushing now runs only after durable CORE review settlement and post-CORE review compatibility persistence, uses a review-hash idempotency suffix, and delegates to a runtime-owned `commandBearingReviewPromptFlusher` that calls the real prompt sync path. The review flusher uses non-adopting runtime prompt sync (`commitRuntimeState:false`) and now passes a scheduler-owned `beforeInstallPrompt` freshness guard through FORGE/runtime into LENS, so stale source state blocks host prompt installation before LENS writes prompt blocks. Stale returned or pre-install-rejected state is skipped with compact `DIRECTIVE_SIDECAR_POST_SETTLEMENT_STATE_STALE` warning evidence. Failed review CORE settlement or failed review compatibility persistence blocks review prompt flushing; LENS flush failure is compact warning evidence and does not roll back the already-applied review mutation. The scheduler-owned review `syncPromptContext(...)` path is fallback-only when the FORGE/LENS helper is absent, not after helper failure.
...
19. Completed bridge constraint: post-CORE compatibility projection persistence failure no longer pushes ordinary accepted sidecar prompt synchronization out of the FORGE/LENS owner path. If a FORGE/LENS accepted-batch flusher is available, it still receives the in-memory projected campaign state plus compact CORE accepted-batch projection after durable CORE/FORGE settlement, and scheduler-owned `syncPromptContext(...)` remains unavailable. The old projection persist failure stays compact `oldProjectionPersist` warning evidence. Command Bearing review prompt synchronization remains separately gated by post-review compatibility persistence.
...
21. Next bridge slices: continue removing the temporary v1 compatibility projection/journal bridge only after CORE-backed read projections can feed runtime surfaces, broaden location-transition/time-boundary trigger coverage beyond committed visible turns, and add auxiliary seal/digest/recall artifact storage beyond refs where needed.
...
- accepted worker outputs validate as one operation bundle, then assign compatibility projection only after FORGE/CORE settlement;
