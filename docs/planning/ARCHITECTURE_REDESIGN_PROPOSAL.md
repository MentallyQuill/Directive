...
This document is not a compatibility plan. Directive is pre-alpha, so the redesign should replace the current save/runtime shape in place. Compatibility migration, old-save preservation, and old runtime authority are out of scope unless a current production path truly needs a temporary bridge to reach the new owner.
...
## Frozen Redesign Definition

The redesign is frozen to five exits. New work belongs in backlog unless it directly unblocks one of these exits:

1. CORE/v2 authority for active turns, timing, save heads, resume, and 5000-message scale.
2. REPAIR/SRE authority for source mutation, selected-swipe changes, edits, deletes, retries, reruns, rollbacks, reobserve, post-visible source review, and stale-source prevention.
3. FORGE/LENS ownership for background generation/effects, prompt dirty domains, prompt cache keys, prompt budget lanes, external prompt environment refs, prompt revision records, and prompt-key preservation.
4. External-context evidence contract for ST Lorebooks/World Info, Memory Books, Summaryception, and VectFox as external influence proven through bounded redacted evidence, not Directive-owned truth.
5. Wave 4 proof after the first four exits are implemented.

Implementation complete means exits 1-4 are implemented and deterministic/integration evidence proves the owner boundaries. Live certified means exit 5 has validated those finished paths. Live proof should not keep expanding the redesign.

This list is now the redesign scope boundary. Preserving old runtime authority, old save payload authority, old ledgers, or stale compatibility fixtures is not a sixth exit. If current production code still needs an old projection, it must be named as bridge telemetry with a removal gate; otherwise tests and docs should be rewritten to the relevant CORE/v2, REPAIR/SRE, FORGE/LENS, or external-context evidence owner.
...
- Directive is pre-alpha, so the redesign may replace the old runtime/save architecture in place. Do not preserve old write paths, old helper APIs, old runtime ledgers, old save payload shapes, or stale compatibility tests as long-term requirements.
- Compatibility migration is not a redesign requirement. Temporary bridges exist only to keep a current production path moving to CORE/v2, REPAIR/SRE, FORGE/LENS, or the external-context evidence contract, and each bridge needs an owner and removal gate.
- Stale compatibility tests must be converted to owner assertions. If a test requires `runtimeTracking`, v1 payload shape, old ledgers, old helper names, compatibility imports, or old recovery/sidecar/model-call journals as authority, rewrite it to assert CORE/v2, REPAIR/SRE, FORGE/LENS, or external-context evidence behavior instead of making new code satisfy old ledgers.
- Old runtime authority should stop at the first proven replacement owner. A current production path may keep an old projection only as named bridge telemetry with a removal gate; unnamed compatibility preservation is out of scope.
- Implementation complete and live certified are separate states. Exits 1-4 define implementation completion; Wave 4 proves those finished paths and must not become a new redesign-discovery lane.
...
- During cutover, any dual-write bridge must be consistency-safe in both directions. A bridge may temporarily project between old state and CORE/FORGE/LENS, but it cannot commit old state ahead of the new owner or the new owner ahead of old projection without a tested preflight, transaction, compensation, or recovery contract. Prefer deleting old authority over making a permanent bridge safer.
...
| 2026-07-05 | Freeze redesign definition to five exits and drop compatibility migration as a requirement. | Old runtime authority and stale compatibility tests must be removed, fail-closed, or rewritten to CORE/v2 owner assertions; implementation complete and live certified are separate states. | Active. |
...
| July 2 bounded five-user Ashes proof | Artifact `artifacts/live-soak/continuity-projection-matrix-five-user/2026-07-02T22-14-31-788Z/report.json` returned `ok: true` / aggregate `warning` after `--resume`, using only `directive-soak-a` through `directive-soak-e` and leaving `default-user` out of soak proof. It passed external-context readiness proof, continuity prompt/source proof, external-context generation proof, persisted CORE generation-start timing, persisted CORE host-native completion, durable model-call failure policy, factual grounding, and story-quality review across all five lanes. A first pass exposed one real host-native completion miss (`hostContinueReleased` without visible assistant completion) in `ashes-sidecars-timekeeping`; the resumed lane passed. Follow-up strict preflight with `--coverage-standard all-lanes` now passes aggregate/lane execution, `external-context-coverage-standard`, factual grounding, CORE timing, CORE host-native completion, story quality, and model-call failure policy by recomputing readiness fixture depth from the raw readiness probe and accepting Memory Books active-book metadata plus valid range diagnostics when no `1_memory` prompt key appears. | Bounded five-lane proof is now green enough to guide implementation, but it is still not architecture completion. Full certification still needs 52 turns per lane, full prompt/fact-check/story-review artifact depth, full external-context generation depth, and selected mutation actuation proof. The SRE latest-pair timeout is capped to 8 seconds and release policy treats that role's fail-closed timeout as fallback-handled only because no state, prompt, or prose authority is lost. |
...
The release proof must therefore be actuation proof, not discovery proof. A script that only finds SillyTavern edit/delete/swipe controls proves host affordances exist; it does not prove Directive can survive the mutation. Certification needs a single message-mutation artifact that records non-human-user host actuation for source edit, source delete, assistant edit, assistant delete, and selected-swipe source truth. Edit/delete scenarios must show the native host control moved and include a compact `directive.sourceMutationProof.v1` block with REPAIR/CORE recovery refs. The selected-swipe runtime path now accepts host `MESSAGE_SWIPED` events and records hash-only REPAIR/CORE source-mutation evidence. `run-sillytavern-selected-swipe-actuation-live.mjs` is now the dedicated Playwright runner for the missing release path: it opens explicit non-human Ashes save/chat bindings, prepares an unselected candidate swipe on a recorded Directive assistant response, clicks a native SillyTavern swipe control, observes the selected index change, and emits `actuationMode: "native-host-swipe-control"` plus `nativeHostControlMoved: true`. July 2 live attempts refined the proof boundary: host-native assistant rows are not valid if only raw chat output exists, older recorded Directive rows can accept candidate swipes but cannot certify native text-button actuation because SillyTavern binds text swipe buttons only to `.last_mes`, and the runner now fails before candidate append unless the latest visible row is CORE/ledger recorded. The runner now includes an opt-in setup mode that posts a setup-only latest Directive-owned response through the host adapter, records the matching response-ledger target through the runtime test hook, and reports only setup ids/hashes/counts. Host message ids are positional, so setup mode uses the exact setup response id instead of trusting a single host-id map that may collide with older ledger rows. The runner also dismisses the Directive preset-update overlay before clicking native host controls, because that overlay can cover the SillyTavern swipe button and make discovery look like actuation. Strict setup-assisted live proof passed on `directive-soak-b` at `artifacts/live-smoke/selected-swipe-actuation/2026-07-02T23-41-07-914Z/selected-swipe-actuation-report.json`: served extension fresh, `defaultUserTouched: false`, setup latest host row `45`, native `.last_mes .swipe_right` hit-test clean, `MESSAGE_SWIPED` observed, selected index moved `0 -> 1`, `nativeHostControlMoved: true`, and no diagnostic API fallback. Final natural latest-response proof then passed on `directive-soak-b` at `artifacts/live-smoke/selected-swipe-actuation/2026-07-03T00-11-15-605Z/selected-swipe-actuation-report.json` after a real Ashes turn produced a CORE-recorded host-generation assistant row: served extension fresh, `targetSetup: null`, `defaultUserTouched: false`, latest host row `51`, `.last_mes .swipe_right` clicked by Playwright locator, selected index moved `0 -> 1`, `nativeHostControlMoved: true`, `sourceIntegrity: "clean"`, and no diagnostic fallback. Correct-as-Swipe and the SillyTavern adapter now treat CORE-recorded host-generation responses as eligible sources without requiring setup-only Directive metadata, while arbitrary unrecorded assistant rows remain blocked. The strict actuation preflight rejects staged `actuationMode: "staged-context-source-truth"` evidence as release proof, while still letting that staged evidence serve as a regression tool. All scenarios must keep prompt and transaction refs source-bound, redact raw replacement or prompt text, check served extension freshness, and avoid `default-user`.
...
- False-belief and inferred facts are perspective/provisional context, not protected objective truth. They may render only for actors allowed by `knownBy`/`witnessedBy`, must carry lower confidence, must not become contradiction guards, and must not displace source-backed objective facts with the same conflict key.
- Legacy visibility labels must migrate into witness semantics while materializers are being enriched. `directorOnly` and `hidden` imply `secret` unless a richer disclosure state is provided; `narratorSafe` and `playerFacing` remain `public`. Group ids in `knownBy` and `witnessedBy` must expand through the current Frame's actor group map before a fact can enter active-cast or protected-continuity prompt lanes.

Implementation evidence from July 2: `createContinuityFact(...)` normalizes witness fields, strips raw evidence refs, derives `secret` disclosure from legacy `directorOnly`/`hidden` visibility labels, caps inferred confidence at `0.7`, and caps false-belief confidence at `0.5`. `buildContinuityFactIndex(...)` preserves actor-scoped false beliefs beside objective facts without letting them replace objective truth, while inferred records lose precedence ties to asserted records. `buildContinuitySourceFrame(...)` preserves scene actor group maps, and `validateContinuityProjectionPlan(...)` applies source-Frame actor/group scope to selections, hard-floor insertion, guard focus, and compression groups; false-belief/inferred facts are lowered out of `directive.continuity.invariants` and rejected for `guardOnly`. `buildContinuityProjectionMatrix(...)` now emits compact per-fact LENS budget refs so actor/group-scoped witness facts count against `activeCast` while objective continuity remains in `protectedContinuity`; `createLensPromptScheduler(...)` consumes those block-level refs for budget traces and strips them from returned/installed packets after trace construction. Focused proof lives in `test-continuity-projection-foundation.mjs`, `test-continuity-projection-plan-validator.mjs`, and `test-lens-prompt-budget-lane-contracts.mjs`, with adjacent proof in `test-player-safe-prompt-context.mjs`, `test-continuity-director-packets.mjs`, `test-storage-scale-5000.mjs`, `test-architecture-redesign-system-skeletons.mjs`, `test-prompt-dirty-domains.mjs`, `test-chat-native-runtime-flow.mjs`, and `test-runtime-host-injection.mjs`.
...
- Old-save import tooling is historical/dev-only and is no longer an active redesign exit. Current storage proof comes from CORE/v2 commits, active-save v2 persistence, scale fixtures, and fail-closed v2 authority tests.
...
- `tools/scripts/test-storage-scale-5000.mjs`, `tools/scripts/test-open-world-event-reducers.mjs`, and `tools/scripts/test-turn-commit-coordinator-core-mechanics.mjs` verify broad `rootsSet` pressure is rejected or excluded from active CORE/v2 hot artifacts.
...
- `repair-runtime.mjs` now has a tested CORE recovery bridge shape for committed player-message edits/deletes and Directive-response edits/deletes. The bridge writes sanitized source-mutation metadata with ids, hashes, dependent refs, source Frame refs, compact repair decisions, REPAIR-owned repair projections, allowed actions, and pre-outcome revision, while excluding raw replacement text from CORE.
...
- CORE/v2 can write/read bounded heads, manifests, segments, checkpoints, and hot appends without full-save rewrites;
