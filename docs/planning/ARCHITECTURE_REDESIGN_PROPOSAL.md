...
The first SRE extraction slice now moves the host-native continuity verdict itself behind `source-reconciliation-engine.mjs`. Response dispatch no longer imports or calls the continuity contradiction guard directly for host-native observations; it calls the SRE facade, and an injected SRE verdict is authoritative. The default facade preserves the old `ok/findings/checkedFactCount` contract while adding compact owner/source metadata. This is deliberately only the verdict boundary: response dispatch still owns post-verdict recovery/quarantine/projection side effects until a later `sourceSettlementService`/SRE settlement slice returns terminal settlement decisions for host-native contradiction.
...
The latest-pair preflight slice now gives SRE the first blocking selected-variant guard before Scene Handshake provider work. Chat ingress Frames carry the previous non-system, non-Directive assistant selected-swipe hash and source-integrity status. Scene Handshake still owns its legacy settlement/apply path, but it cannot call the provider or apply proposals when SRE preflight returns non-clean source status. This is intentionally not terminal latest-pair cutover: `settleLatestPair(...)` cannot report accepted operation authority without an apply owner, and the next cutover must put terminal latest-pair settlement inside a single Scene Handshake/SRE owner boundary so SRE and legacy Scene Handshake do not both mutate state, prompt context, or time advancement.
...
- The implementation target is a facade such as `src/runtime/source-settlement-service.mjs`, initially dispatching to mode modules while shared source-frame resolution and integrity checks move behind the service boundary. Explicit-range terminal settlement has started moving behind this service for Scene Reconciliation, with hash-only persisted evidence and fail-closed apply semantics. Latest-pair selected-variant preflight now blocks stale/mismatched source before Scene Handshake provider work, but terminal latest-pair settlement remains open until it can replace the legacy Scene Handshake apply path without double-settlement. The current `src/runtime/source-reconciliation-engine.mjs` is the first narrow facade for host-native continuity verdicts and should either become a mode inside that service or be renamed/absorbed when latest-pair and host-native terminal settlement move behind the same boundary.
