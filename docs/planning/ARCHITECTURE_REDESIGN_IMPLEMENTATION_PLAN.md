...
Updated June 30, 2026. This section is the current operating board until Agent-0 replaces it with a fresher Architecture Redesign Board entry. It exists so future workers do not mistake architecture capture, bounded proof, or partial bridge safety for completed implementation.

| Lane | Current state | Next execution action | Do not spend time on |
| --- | --- | --- | --- |
| CORE/v2 runtime authority | Partly implemented and focused-test proven for first-turn bootstrap, source-frame scope validation, hot `beginTurn(...)` append, v2 bridge load/recovery, manifest freshness, Save/Save As/autosave v2 authority, response duplicate recovery, and CORE-projection freshness arbitration. Remaining blockers are full runtime resume semantics, broader durable response/recovery/background lifecycle ownership, final independent SRE review-worker ownership, and final bounded reducer/event mechanics application. | Use Wave 1 CORE Runtime/Storage/QA workers only for slices that remove old-ledger durable authority or prove the runtime can resume/replay from CORE/v2 without hot full-save rewrites. | Prompt wording, UI polish, broad live proof, and proof-pipeline refinements unless they expose data loss, source-authority confusion, raw leakage, or a primary vertical-path blocker. |
| REPAIR/SRE ownership | Partly implemented for source-mutation recovery boundary, dependent-edit guard, selected-swipe runtime event handling, latest-source restart links, rollback authorization for source mutation, host-native response-recovery policy seams, and no-CORE `hostResponsePostFailure` retry fail-closed actuation. Remaining blockers are outcome rerun ownership, provider-failure retry/reobserve coverage, broader rollback execution, visibility-only production mutation ownership, Scene Handshake/Mission Component invalidation, committed outcome delete policy, and final SRE-owned review worker separation from response-dispatch bridge settlement. | Use the Wave 2 REPAIR/SRE/mutation QA pack for one policy table or runtime seam at a time; old reconciliation ledgers may mirror results only after REPAIR/SRE decides the action. | Letting edited/deleted/retried sources re-enter normal classification, rerunning mechanics without REPAIR authorization, or treating selected-swipe/correction candidates as accepted continuity before selection. |
| FORGE/LENS ownership | Active Wave 3 cutover. Accepted sidecar bridge safety, scene/phase seals, pressure/arc digests, open-world boundary settlement, internal background settlement, accepted-batch prompt flushing, no-FORGE accepted-batch fail-closed behavior, and Command Bearing review prompt flushing have focused bridge proof, but temporary scheduler/v1 compatibility projection and journal ownership still remain. The accepted sidecar prompt missing-helper/replay quality gate is now closed at focused-test depth: once FORGE participates, missing or falsey accepted prompt helpers record compact LENS-unavailable warning evidence, scheduler-owned `syncPromptContext(...)` does not run, and accepted replay carries the same warning so it cannot become a clean provider-cache barrier. No-FORGE accepted batches now reject before old v1 mutation instead of using scheduler-built direct CORE settlement. Accepted effectful batches now validate the compatibility projection without mutation, settle through FORGE/CORE, then assign/persist the projection after settlement. The Command Bearing review prompt stale-state quality gate is also closed at focused-test depth: production passes `commitRuntimeState:false` and a scheduler-owned `beforeInstallPrompt` freshness guard through FORGE into runtime/LENS prompt sync, so stale source state blocks host prompt installation and returned prompt state is adopted only after scheduler freshness arbitration. | Continue removing the remaining v1 compatibility projection/journal bridge once CORE read projections cover runtime surfaces. Keep LENS work focused on prompt ownership blockers, not generic rebuild polish. | More broad sidecar audits, generic prompt rebuild polish, or full live proof before the remaining v1 projection/journal bridges have owner/removal gates. |
| Narrative Engine transfer | Captured architecturally, not fully complete. Recall Index, LENS prompt budget lanes, witness-scoped facts, scene/phase seals, pressure/arc digests, structured retrieval metadata, optional semantic candidates, and Correct-as-Swipe all have owners and gates, but each row remains incomplete until its implementation detail checklist and proof pass. | Assign Recall/Budget, Witness/Correction, FORGE/LENS, and REPAIR/SRE workers through the transfer matrix; every slice must state owner, runtime path, storage boundary, invalidation behavior, proof gate, and code-reference decision. | Porting Narrative Engine's app/server/storage/UI/vector stack, treating optional semantic/vector hits as authority, or marking a feature complete because it is described in prose. |
...
1. Continue the remaining accepted-sidecar bridge cutover: no-FORGE direct CORE fallback is removed, pre-settlement old apply is gone, and the LENS stale-source host-install race is guarded, so the next ownership work is shrinking the temporary v1 compatibility projection/journal bridge once CORE read projections can feed runtime surfaces without reintroducing split-brain write order.
2. Return to the remaining Wave 1 and Wave 2 authority blockers: full runtime resume semantics, broader durable turn lifecycle ownership in CORE, outcome rerun authorization, provider-failure retry/reobserve coverage, broader rollback execution, visibility-only mutation ownership, and independent SRE review-worker ownership.
3. Finish the Narrative Engine transfer through the owning systems rather than as standalone modules: Recall Index and budget lanes in LENS/CORE, witness/correction in SRE/REPAIR, seals/digests in FORGE, optional semantic candidates behind deterministic-first gates.
...
| Correct-as-Swipe | Captured as Define Selection plus SRE evidence verdict, REPAIR correction case, candidate swipe provenance, and selected-swipe acceptance boundary. | End-to-end correction case state table, candidate swipe append, reject/expire behavior, accepted-continuity non-mutation before selection, and native selected-swipe actuation proof remain open. | Wave 2 and Wave 4. |
...
| --- | --- | --- | --- |
| CORE/v2 runtime authority | Implementation blocker until full runtime resume semantics and broader durable turn lifecycle ownership no longer depend on old ledgers. | Focused runtime/storage tests prove active resume, response/recovery/background lifecycle, Save/Save As/autosave ownership, and 5000-message hot-write/read budget without full-save rewrite. | Agent-0 plus CORE Storage/Runtime workers. |
| REPAIR/SRE ownership | Implementation blocker until generic retry/rerun, rollback, selected-swipe/range review, visibility-only changes, and host-native contradiction review enter REPAIR/SRE before old-ledger mutation. | Deterministic mutation/retry/swipe fixtures prove no mechanics rerun, no dependent-edit reobserve loop, CORE-first recovery, and SRE source verdicts. | Agent-0 plus REPAIR/SRE workers. |
| FORGE/LENS ownership | Implementation blocker until background compatibility projection/prompt/journal ownership is cut over from temporary v1 bridges and prompt rebuilds are coalesced through LENS. | FORGE/LENS tests prove source-current guards, one accepted batch result, stable idempotency, no raw diagnostics, bounded sidecar journals, one prompt rebuild, and prompt-key coexistence. | Agent-0 plus FORGE/LENS workers. |
...
3. Finish Wave 3 FORGE/LENS ownership for background batching, completed accepted-batch bridge-consistency proof, scene/phase seals, pressure/arc digests, recall revisions, prompt budget lanes, prompt cache inputs, and prompt-key coexistence, then cut over the remaining accepted-sidecar compatibility projection/prompt/journal bridge from temporary v1 ownership into FORGE/LENS.
...
| --- | --- | --- | --- |
| Hybrid scene recall | CORE projections plus Directive Recall Index, consumed by LENS/SRE/REPAIR/FORGE | Partially implemented through Recall Index contracts, scene-seal refs, scale fixtures, and LENS cache inputs. Full invalidation/fork behavior and auxiliary recall artifact storage remain open. | Recall entries/query/results use Frame/CORE/package/scene-seal/reviewed-import refs only; rollback, branch, Save As, delete, edit, and selected-swipe changes invalidate or fork recall deterministically; 5000-message scale proof includes recall segments and no raw transcript archive. |
| Prompt budget lanes | LENS | Partially implemented through budget-lane contracts, cache inputs, and trace schemas. Full prompt-builder cutover and overflow acceptance proof remain open. | Prompt builds emit lane budgets, floors, included refs, omitted refs, omission reasons, cache inputs, and external-environment refs; controlled overflow test proves deterministic omission behavior without raw prompt storage. |
| Witness-scoped facts | Continuity schemas, CPM materializers, SRE, FORGE, and LENS active-cast/protected-continuity lanes | Captured in architecture and stage mapping; implementation proof still needs explicit witness-fact tests and prompt inclusion behavior. | `knownBy`, `witnessedBy`, `subjectIds`, `disclosureState`, `disclosureSourceFrameId`, and evidence refs drive continuity materialization and active-cast prompt inclusion; anti-telepathy test proves hidden knowledge is not given to actors who do not know it. |
...
| CORE/FORGE accepted-batch bridge consistency | This blocker is now closed at focused-test depth. The accepted regular sidecar bridge uses a FORGE prepare / non-mutating compatibility validation / final-settle / post-settlement projection protocol: CORE/FORGE rejection fails before old mutation, validation failure fails before final settlement, final-settlement failure no longer requires compensation because the compatibility projection has not been assigned, post-settlement compatibility-persist failure is surfaced as compact warning evidence, accepted-journal persistence failure records a compact `recoveryRequired` marker instead of reporting clean success, identical accepted-batch replay is idempotent, changed accepted-batch replay rejects by worker-results hash even when the caller does not supply an explicit hash, and provider failures/results are compacted before activity, CORE diagnostics, sidecar journals, or replay caches can leak raw provider text. | Treat this as a bridge-safety proof, not a final ownership cutover. Next FORGE/LENS slices should remove the temporary v1 compatibility projection/prompt/journal bridge, keep accepted-batch split-brain and raw-canary tests as the safety floor, and move the scheduler-owned prompt sync/journal projections under the target owners once the full sidecar lifecycle can commit through FORGE/LENS directly. |
| Production FORGE ownership | Synthetic FORGE and the regular campaign-sidecar bridge now prove non-mutating compatibility projection validation, one FORGE-owned accepted-batch CORE background settlement before compatibility assignment, post-settlement compatibility persistence, FORGE/LENS prompt flushing, conflict-before-mutation, bridge-safe accepted-batch prepare/validate/final-settle ordering, request-hash-aware provider replay, hash-guarded accepted-batch replay, compact recovery/warning evidence, and raw-provider-diagnostic redaction for accepted regular sidecars. Production `runtime-app` now injects `createForgeCoordinator(...)` into the sidecar scheduler, and accepted sidecar worker results pass through the FORGE accepted-batch path with provider-owner diagnostics and idempotent replay instead of the scheduler hand-building the CORE background batch. Command Log summary, Narrative Thread extraction, advisory enrichment, and terminal checkpoint settlement remain specialized queues/control paths, but successful settlements now share CORE background semantics instead of diagnostics-only apply. The Narrative Thread bridge now covers normal committed turns and non-terminal pending accept/confirm resolutions; the director still applies internal state commits through its existing director path rather than one FORGE aggregate apply. | Continue moving provider execution, validation, prompt scheduling, journals, director-internal multi-commit work, and remaining background effects into FORGE/background settlement now that accepted-batch bridge safety is proved. Keep regular sidecar, Command Log, Narrative Thread, advisory, terminal checkpoint, accepted-batch split-brain, provider-replay-hash, accepted-replay-hash, recovery-marker, and raw-canary tests as the production safety floor. The next cutover target is making CORE read projections rich enough to delete the accepted-sidecar compatibility projection bridge entirely. |
...
- The test closeout for this production bridge is `test-campaign-sidecar-scheduler.mjs`, `test-background-projection-batch.mjs`, `test-core-store-v2.mjs`, `test-chat-native-runtime-flow.mjs`, `test-chat-turn-orchestrator.mjs`, `test-command-log-summary-sidecar.mjs`, `test-narrative-thread-director.mjs`, and `test-runtime-host-injection.mjs`. The expected evidence is one non-mutating compatibility projection validation for accepted regular sidecars, one CORE background commit when a transaction id is present, post-settlement compatibility persistence, one prompt sync/flush, conflict detection before mutation, idempotent/stale background behavior, Command Log summary remaining post-visible-response while settling successful summaries through CORE background effects, Narrative Thread extraction remaining post-visible-response for normal and non-terminal pending committed turns, cancel/revise paths not scheduling extraction, and stale source guards preventing provider/apply work when possible.
...
- First production bridge slice, completed for regular campaign sidecars:
  ...
  - accepted non-conflicting operations aggregate into one non-mutating compatibility projection preview and one FORGE-owned CORE background batch settlement before compatibility assignment/persistence;
  ...
  - the scheduler records a stable sidecar batch id before provider execution and skips duplicate compatibility projection/prompt/journal/CORE settlement when the same transaction/source/workers are scheduled again;
...
- The scheduler keeps a temporary stable sidecar-batch replay barrier for its old v1 parse/projection/prompt/journal bridge. Replayed provider packets cannot duplicate compatibility projection assignment, prompt sync, FORGE settlement, or sidecar journals.
- The scheduler's accepted-batch prompt bridge now computes LENS-normalized dirty domains from accepted worker roots/operation domains (`ship`/`crew`/`relationships` -> `crewShipRelationship`, `commandBearing` -> `command`, `continuity`/`factIndex` -> `continuity`) and hands stable accepted-batch `promptSyncIdempotencyKey` values to FORGE/LENS after compatibility projection persistence succeeds.
- Accepted regular sidecar prompt synchronization now flows through a FORGE-owned `flushAcceptedBatchPrompt(...)` seam when available. Production runtime wires that seam to a runtime-owned `acceptedBatchPromptFlusher` adapter that calls the real `synchronizeActivePrompt(...)` path, so accepted sidecars do not use LENS's placeholder packet builder and the scheduler can adopt/persist the returned prompt-context campaign state. Command Bearing closure-review prompt synchronization now flows through a FORGE-owned `flushCommandBearingReviewPrompt(...)` seam after durable review CORE settlement and review compatibility persistence. The scheduler-owned `syncPromptContext(...)` paths are fallback-only when the FORGE/LENS helper is unavailable.
...
- REPAIR/CORE response-recovery follow-up, June 30: `chat-turn-orchestrator.mjs` now treats CORE-backed Directive response post failures and provider-failure-after-mechanics recovery marking as CORE-mandatory. If `markRecoveryRequired(...)` is missing or returns `notRecorded` for a turn that already has a CORE transaction, the turn records a `DIRECTIVE_CORE_RESPONSE_RECOVERY_NOT_RECORDED` processing recovery and does not create an old-ledger-only `hostResponsePostFailure` retry record. When CORE accepts the recovery, the old recovery journal carries only compact projection evidence: `coreTransactionId`, `coreRecovery`, and REPAIR `repairDecision`. Focused proof: `test-chat-turn-orchestrator.mjs` covers the missing-writer fail-closed case and the provider-failure projection case.
- REPAIR response-retry actuation follow-up, July 1: `retryCommittedResponse(...)` now fails closed whenever REPAIR does not authorize the retry, including old `hostResponsePostFailure` recovery rows with no CORE transaction. The regression test temporarily proved the old guard returned `ok: true` for a no-CORE retry, then the restored guard returned `response-retry-not-authorized` without response dispatch, recovery resolution, or persistence. Focused proof: `node tools/scripts/test-chat-turn-orchestrator.mjs` plus syntax checks for `src/runtime/chat-turn-orchestrator.mjs` and `tools/scripts/test-chat-turn-orchestrator.mjs`. Remaining REPAIR/SRE actuation blockers are outcome rerun through `previewOutcomeReplacement(...)`, provider-failure retry/reobserve coverage, broader rollback execution, and final SRE review-worker ownership.
...
- Accepted regular sidecar bridge-safety closeout, June 30: accepted effectful regular sidecar batches now use a FORGE prepare / non-mutating old projection validation / final CORE settlement / post-CORE compatibility assignment and persist protocol instead of treating every worker as an independently durable v1 sidecar result. `campaign-sidecar-scheduler.mjs` computes one accepted-batch hash from the accepted worker results, requires final settlement evidence to match the CORE transaction id, background batch id, idempotency key, and `acceptedBatchHash`, and no longer lets the old compatibility projection mutate or persist before durable CORE/FORGE settlement exists. If the post-CORE compatibility persist fails, prompt sync is skipped and compact warning evidence is returned instead of raw errors. FORGE computes accepted-batch hashes from worker results when present, rejects supplied/computed hash mismatches, and rejects effectful cached replay without matching accepted-batch evidence. CORE returned and hydrated `backgroundBatches[]` now preserve compact `forgeBatchRef`, `backgroundEffectRefs`, `acceptedBatchHash`, `reviewHash`, and safe read-model refs for scene seals, pressure digests, recall entries, and open-world boundary settlements without raw provider/review text. Command Bearing closure review mutations now require a separate CORE background settlement with `reviewHash` validation before review-specific prompt sync. Focused proof: `test-campaign-sidecar-scheduler.mjs` covers no old v1 success/noChange journaling on accepted settled batches, no pre-CORE old projection mutation or persist, post-CORE compatibility-persist warning, unsafe preflight `settled`/`noChange`, missing/mismatched transaction/batch/idempotency/hash receipts, no-evidence replay rejection, supplied-hash spoof rejection, validation/final-settlement failure ordering, accepted-journal persistence bypass, Command Bearing review settlement failure, and raw warning/reason redaction; `test-core-store-v2.mjs` covers production CORE returned/hydrated compact refs; `test-forge-open-world-boundary-settlement.mjs` covers identical replay versus same-idempotency mismatched replay; `test-forge-scene-phase-seal.mjs` and `test-forge-pressure-arc-digest.mjs` cover derived read-model refs after compaction. This remains a temporary bridge, not final ownership: the remaining Wave 3 cutover is to remove the accepted-sidecar v1 compatibility projection/journal bridge once CORE read projections cover runtime callers.
- Accepted regular sidecar prompt ownership follow-up, June 30: accepted sidecar prompt synchronization now moves behind FORGE/LENS without enabling premature LENS flush during final CORE settlement. After the post-CORE compatibility projection persist succeeds, `campaign-sidecar-scheduler.mjs` calls `forgeCoordinator.flushAcceptedBatchPrompt(...)` with normalized dirty domains, a prompt idempotency key derived from provider request evidence plus the accepted worker-result hash, source refs, and the compatibility campaign state. `forge-coordinator.mjs` exposes this as an injected `acceptedBatchPromptFlusher` seam, and production `runtime-app.mjs` wires that seam to `synchronizeActivePrompt(..., persist: false)` so the real LENS packet builder and prompt-context metadata path are used instead of the scheduler calling `syncPromptContext(...)` or FORGE calling LENS's placeholder packet builder. The scheduler adopts and persists returned campaign state, does not fall back after a FORGE/LENS flush attempt or failure, and still blocks all prompt synchronization if post-CORE compatibility persistence fails. Completed provider-batch replay now caches only clean applied outcomes; rejected bridge outcomes, recovery-required results, post-settlement warning results, and unsafe FORGE statuses cannot become replay barriers. Once FORGE participates in accepted-batch lifecycle, missing or falsey accepted prompt helpers now record compact LENS-unavailable warning evidence instead of invoking scheduler-owned `syncPromptContext(...)`; replay carries the same warning so it cannot become a clean provider-cache barrier. No-FORGE accepted batches now reject before old mutation, so there is no no-FORGE prompt fallback. Focused proof: `test-campaign-sidecar-scheduler.mjs` covers post-persist FORGE/LENS accepted-batch prompt flush, returned campaign-state adoption, stable dirty-domain/idempotency evidence, changed accepted worker-result hashes, changed provider request hashes, no scheduler prompt-sync call in the FORGE/LENS path, no fallback after LENS failure, missing FORGE/LENS helper warning evidence, replay warning preservation, transient rejected bridge outcomes retrying instead of replaying as completed, compatibility-persist failure blocking prompt sync, and raw source canaries excluded from FORGE/LENS prompt evidence. Syntax proof: `node --check src\jobs\campaign-sidecar-scheduler.mjs`, `node --check src\jobs\forge-coordinator.mjs`, and `node --check src\runtime\runtime-app.mjs`.
- Command Bearing review prompt ownership follow-up, June 30: Command Bearing closure-review prompt synchronization now moves behind FORGE/LENS as its own post-review prompt adapter. The scheduler still performs the temporary review mutation and post-CORE compatibility projection persist, but only after durable CORE review settlement and compatibility persistence does it call `forgeCoordinator.flushCommandBearingReviewPrompt(...)` with `promptDirtyDomains: ['commandBearing']`, source refs, campaign context, and a prompt idempotency key suffixed by the validated `reviewHash`. `forge-coordinator.mjs` exposes this as an injected `commandBearingReviewPromptFlusher` seam, and production `runtime-app.mjs` wires that seam to `synchronizeActivePrompt(..., persist: false)` with a Command Bearing review prompt frame. Failed review CORE settlement and failed review compatibility persistence block review prompt flushing. A FORGE/LENS review prompt flush failure records compact `lens` warning evidence and leaves the already-applied review mutation intact. The scheduler-owned review `syncPromptContext(...)` path remains fallback-only when the FORGE/LENS helper is absent. Focused proof: `test-campaign-sidecar-scheduler.mjs` covers review CORE receipt `reviewHash` validation, review prompt flush ordering after review settlement and compatibility persist, review-hash idempotency, no scheduler review prompt sync in the FORGE/LENS path, no fallback after LENS failure, compatibility-persist failure blocking review prompt flush, LENS failure warning semantics, and raw warning/canary redaction. Syntax proof: `node --check src\jobs\campaign-sidecar-scheduler.mjs`, `node --check src\jobs\forge-coordinator.mjs`, and `node --check src\runtime\runtime-app.mjs`.
...
- Accepted sidecar compatibility-persist prompt-sync follow-up, July 1: this supersedes the earlier June 30 accepted sidecar prompt-ownership note that treated post-CORE compatibility projection persistence as a hard gate for all prompt synchronization. After durable CORE/FORGE settlement, ordinary accepted-batch prompt sync may now proceed from the in-memory projected campaign state even if the old compatibility persist emits compact `oldProjectionPersist` warning evidence. That keeps LENS prompt freshness tied to CORE/FORGE settlement and compact accepted-batch projection evidence instead of a best-effort v1 compatibility write. Command Bearing closure-review prompt sync remains gated by its separate post-review compatibility persistence path because that path performs a second review mutation. Focused proof: `test-campaign-sidecar-scheduler.mjs` first failed with `postCorePromptSyncs.length === 0`, then passed with the new `postCorePromptSyncs.length === 1` assertion while preserving the Command Bearing no-sync assertion. Adjacent checks passed for `test-prompt-dirty-domains.mjs`, `test-chat-native-runtime-flow.mjs`, `test-background-projection-batch.mjs`, `test-active-save-facade-v2.mjs`, `test-storage-cross-writer-v2.mjs`, `test-runtime-host-injection.mjs`, and `test-storage-scale-5000.mjs` with 5000 messages, a 22081-byte materialized head, and no sealed segment rewrites.
- Accepted sidecar FORGE/LENS warning-path ownership tightening, July 1: the compatibility-persist warning path now stays on the FORGE/LENS accepted-batch prompt flusher when that owner is available instead of falling back to scheduler-owned `syncPromptContext(...)`. The flusher receives the in-memory projected campaign state plus compact `directive.coreAcceptedSidecarBatchProjection.v1` evidence after durable CORE/FORGE settlement, then the scheduler persists only the returned prompt-context state. Missing/unavailable FORGE/LENS helpers still produce compact LENS-unavailable warning evidence, and Command Bearing closure-review prompt synchronization remains gated by its separate post-review compatibility persistence path. Focused proof: `test-campaign-sidecar-scheduler.mjs` failed with only the old projection-persist reason recorded, then passed with one FORGE/LENS flush, zero scheduler fallback sync calls, compact CORE accepted-batch projection evidence, and preserved Command Bearing no-sync behavior.
...
The proof producer must validate more than those top-level fields. Edit/delete child reports must include `directive.sourceMutationProof.v1`: target host id, source role, tracked ingress/response kind, mutation kind, original/replacement text hashes or delete original hash, native host-control evidence, ledger status after mutation, CORE recovery status/case refs, compact REPAIR decision kind/action/event type, prompt-context delta, and recovery-journal delta without raw replacement text. Selected-swipe child reports must include `directive.sourceIntegrityProof.v1`: selected host assistant source id, selected swipe index, swipe count, source-integrity status, selected-hash equality with the accepted assistant variant, absence of discarded-swipe canaries from committed state, SRE/Scene Handshake decision status, and source hash refs. Current selected-swipe proof is staged Scene Handshake source-truth evidence; native swipe-control actuation remains a future runner and should not be claimed by the release artifact yet. Shallow scenario shells are acceptable only as release-bundle summary inputs after the strict producer has already validated the child reports.
