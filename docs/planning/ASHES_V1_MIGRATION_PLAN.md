...
Open Orders I targets `chapter-3-dead-letters`. Chapter 3 remains durably pending because it has no exact V1 definition; legacy package data cannot activate in its place. The immediate content migration handoff is therefore Chapter 3 under the same spoiler-safe, aggregate, source-custody, player-agency, and exact-target contracts. Archived Prelude, Chapter 1, Chapter 2, and Open Orders I definitions must remain available at their pinned versions so repaired saves can reconstruct the journey.
