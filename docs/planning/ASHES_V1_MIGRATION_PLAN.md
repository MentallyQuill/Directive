...
Old Lessons targets the source-authored second quiet interval through the V1-only identity `open-orders-2-what-survives`. That interval remains durably pending until its exact definition exists; the runtime neither invents a legacy quest row nor skips directly to Chapter 6. Archived Old Lessons definitions must also remain available at their pinned version for journey reconstruction.
