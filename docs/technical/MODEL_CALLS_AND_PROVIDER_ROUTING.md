...
Structured output is parsed, size-limited, shape-validated, and checked against a closed mission candidate set plus the bounded elapsed-time schema. Provider failure cannot produce partial semantic mutation. Mission evidence and elapsed story time both fail closed; deterministic runtime code alone calculates and commits the canonical Stardate and ship clock. Character drafting may use the local authored fallback.
