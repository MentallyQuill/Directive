...
- When the pointer crosses the midpoint of the next person card, `relocatePlaceholder()` applies the `170ms` FLIP animation to both the displaced card and the destination slot. They move in opposite directions and overlap while crossing, so the list appears to flicker or panic at the threshold.
...
- A pointer held at a peer boundary must not cause the slot and peer to overlap. The slot may touch the peer at a shared edge but must not cross it.
...
- A real Chromium pointer drag of the player-character card across the next card must show zero positive vertical overlap between the settled slot and the displaced peer throughout the reflow.
- The slot's own Web Animations list must stay empty while at least one displaced person card has a running reflow animation.
