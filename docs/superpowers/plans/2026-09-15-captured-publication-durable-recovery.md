...
- [ ] Run focused tests, full gate on final source, scoped diff checks. Record actual scope and publish under standing authorization; keep unrelated debug.log untouched.
- [ ] After publication, verify installed identity and a bounded captured-intent restart scenario in an isolated disposable storage namespace; do not enable live campaign capture or relax branch refusal.
...
Final source passed all240checks and independent integrated review; native browser-restart helper remains in preparation, unexecuted.
