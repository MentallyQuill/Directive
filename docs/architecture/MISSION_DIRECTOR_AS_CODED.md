...
The current implementation proves two deterministic end-to-end turn paths:
...
- Mission-abandoning move: attempt to leave the Hesperus diversion, requiring Captain authority while existing pressures remain active.
...
- `src/adjudication/action-resolver.mjs`: chooses the outcome band, costs, revealed facts, and Command Moment awards.
...
- Scores cadence state, active decision-point links, player intent, and Command Moment availability.
...
- `outcomePacket.commandMomentAwards`: Resolve award for `command.hesperus-fraud-accountability`

The state delta commits:

- Hesperus resolution: `passengers-transferred`
- Arrival delay: `minor`
- Hesperus Command Moment: `resolve-awarded`
...
When the player attempts to leave the active mission area during the Hesperus diversion, the loop produces:

- `intentParse.primaryIntent`: `leave-mission-area`
- `actionClassification.category`: `missionAbandoningMove`
- `authorityCapabilityCheck.result`: `captainDecisionRequired`
...
The current outcome does not let the ship leave automatically. It records that Captain Whitaker must make or approve the command decision and that the original mission pressure continues during deliberation.
...
- Mission-abandoning moves are classified and capability-checked, but do not yet have complete Captain approve/refuse/counteroffer negotiation variants.
