...
A mission definition is an authored rule graph, not a linear quest script. It declares facts, evidence policies, events, outcomes, objectives, clocks, Duty Reports, entry capabilities, closure rules, terminal dispositions, and transitions.
...
Clocks start, pause, resume, expire, and resolve through authored predicates. Only player-known clocks appear in the UI. Time advances from accepted story evidence, not wall time. Expiry consequences are deterministic and can produce fair informed failure only when the deadline was known.
