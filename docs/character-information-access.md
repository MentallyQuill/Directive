...
Access records are statement-specific. A briefing about standby shuttles does not deliver an unmentioned staffing request or deadline. A current claim about an earlier conversation does not establish the earlier conversation. Private thoughts, out-of-character instructions, unshown documents, and unestablished speakerphone audiences are not deliveries. Direct player-controlled speech can establish access when the player enacts its delivery, including setup the player directly enacts in the same turn. If delivery depends on an NPC response or environment change that is neither already enacted in the accepted assistant source nor directly enacted by the player, it remains a proposed or conditional delivery until a later accepted source depicts it. Audience interpretation is a semantic model responsibility; validators prove source custody and shape, not the meaning of arbitrary prose.
...
The projection reads archived addFact events rather than the world-fact projection. Thus a new global estimate does not erase a recipient's earlier estimate, and resolving/dormant threads does not erase acquired information.

Within a turn, acquisition follows the latest required exact statement or audience passage, with the accepted assistant before the current player. Passage ends locate the boundary so a longer quote can disambiguate a later delivery without moving it before preceding speech. An earlier private dispatch read aloud later is therefore prioritized by that delivery, not its original provenance. Several statements sharing one receipt retain their primary passage sequence within that occasion; this does not claim separate receipt times. Presence-versus-delivery interpretation remains the analyst's semantic responsibility. Repeated evidence that leaves the acquisition boundary ambiguous is rejected and requires a longer exact quote; repeated older provenance is allowed when a unique later receipt fixes the boundary.

The existing event array persists this order through reload, pruning and branch rebinding. No schema fields, timestamps or event-identity inputs are added. Legacy records retain their stored order; old receipt positions are not reconstructed from unavailable source text.

Thread status mutations likewise follow source and passage order after creation/fact dependencies. Contradictory mutations for one thread in the same source are rejected when their evidence overlaps or repeats and cannot establish their order. An accepted assistant resolution followed by a player reopening leaves the thread active; player speech still cannot certify resolution or expiry.
