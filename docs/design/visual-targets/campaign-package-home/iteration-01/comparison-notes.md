...
- Real bottom-route clickthrough passed for Campaign, Mission, Crew, Ship, Log, and Settings with non-empty panel content.
...
- Campaign hierarchy: status blocks first, package identity, one primary command zone, readiness rows, then secondary package/library actions.
...
- Tighten the phone Campaign card density. The primary command is readable, but the readiness and metadata rows need more breathing room in the next pass.
...
- Tune responsive Campaign spacing and long-label handling after Mission has a matching shell vocabulary.
