...
Directive bundles cleanup scripts under `extensions.regex_scripts`, the same SillyTavern preset field used by the local Pura and Celia reference presets. When allowed in SillyTavern, the scripts repair common mojibake dash, quote, apostrophe, ellipsis, nonbreaking-space, replacement-character, and zero-width text artifacts before the attached cleanup passes normalize extra spaces, smart quotes, apostrophes, em dashes, ellipses, and CJK characters.

This regex bundle is a deterministic text-cleanup layer. It does not repair, validate, import, or rewrite Lorebooks, World Info, Memory Books, Summaryception, VectFox, or other external context-extension content, and it does not replace provider transport debugging if a model gateway is corrupting UTF-8.
...
- Directive-owned prompt packet rebuild, clear, inspection, and chat-switch suspension.
